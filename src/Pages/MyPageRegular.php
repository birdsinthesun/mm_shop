...
namespace Bits\FlyUxBundle\Pages;
 ...
 */
class MyPageRegular extends Frontend
{
	/**
	 * @var Template
	 */
	...
	private function prepare($objPage)
	{
        ...
        $arrMapper = array();
        $countContentAnchor = 0;

		// Filter the disabled modules
		$arrModuleIds = array_map(
            fn($module) => (int) $module['mod'], // Extrahiere die 'mod' ID
            array_filter($arrModules, fn($module) => $module['enable'] ?? false) // Filtere nach 'enable'
        );
		...
		$objModules = ModuleModel::findMultipleByIds($arrModuleIds);
            ...
            if ($objModules !== null) {
                ...
                foreach ($objModules as $module) {
                    $arrModulesById[$module->id] = $module;
                }
            }
